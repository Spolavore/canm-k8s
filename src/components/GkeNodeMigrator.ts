...
class GkeNodeMigrator {
    ...
    isNodeCordoned(nodeName: string): boolean {
        return this.k8sClient.isNodeCordoned(nodeName);
    }
    ...
    getClusterNodePoolInfo(): Array<any> {
    ...
    private removeNode(nodeName: string, nodePool: string): boolean {
        ...
        const removeSucced = this.removeMigInstance(nodeName, this.getInstanceGroup(nodePool)[0], instanceZone);

        return removeSucced;
    }
    private removeMigInstance(nodeName: string, instanceGroup: string, zone: string): boolean {
        try {
            execSync(`
          ...
          `);
            ...
            logger(
                COMPONENT,
                `Error while trying to remove ${nodeName} node on ${instanceGroup} instance group: ${error}`,
                'info',
            );
            return false;
        }
      ...
      `);
            ...
            if (!this.k8sClient.waitUntilNodeReady(instanceName)) {
                ...
                if (!cleanedUp) {
                    logger(
                        COMPONENT,
                        `CRITICAL: failed to clean up orphan MIG instance ${instanceName}. Manual intervention required.`,
                        'error',
                    );
                }
                return null;
    ...
    async getClusterNodes(): Promise<ClusterNodes> {
        const allNodes = await this.k8sClient.listNodes();
        const createdByCanm: KubernetesNodes[] = [];
        const createdByProvider: KubernetesNodes[] = [];

        for (const n of allNodes) {
            const nodePool = n.labels[GKE_NODEPOOL_LABEL];
            const node: KubernetesNodes = {
                name: n.name,
            ...
            if (n.name.startsWith(CANM_NODE_PREFIX)) {
                if (!nodePool) {
                    logger(
                        COMPONENT,
                        `CANM-prefixed node ${n.name} has no ${GKE_NODEPOOL_LABEL} label; skipping`,
                        'error',
                    );
                    continue;
                }
                createdByCanm.push(node);
            } else {
                createdByProvider.push(node);
            }
        }

        return { createdByCanm, createdByProvider };
    }

    private getInstanceGroup(nodePool: string) {
        const nodePools = this.getClusterNodePoolInfo();
    ...
    annotateNode(nodeName: string, key: keyof typeof ANNOTATION, value: string): boolean {
        return this.k8sClient.annotateNode(nodeName, key, value);
    }
